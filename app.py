...
if "show_nearby" not in st.session_state:

    st.session_state.show_nearby = False
...
st.markdown(
    ...
    .nearby-columns div[data-testid="stButton"] button{
    ...
    with st.popover("⚙"):
        ...
        if hasattr(st, "pills"):
            ...
            selected_routes = (
                ...
                if not route_choice or "全路線" in route_choice

                else [x for x in route_choice if x in route_options]
...
if not df.empty:
    ...
    if rent_filter == "100%未満":

        df = df[df["相対家賃"] < 100]

    elif rent_filter == "100%以上":
...
if nearby_count and not st.session_state.show_nearby:
    ...
    notice_column, button_column = st.columns([5, 1])
