...
def route_parts(route):
    ...
    return list(dict.fromkeys(x.strip() for x in text.split("→")))
...
def card_background(route):
    ...
    for index, color in enumerate(colors):
        sections += [
            f"{color} {index * width:.0f}%",
            f"{color} {(index + 1) * width:.0f}%",
        ]
...
st.markdown(
    """
    <style>
    /* Streamlit上部の固定ヘッダー分を含め、十分な余白を確保 */
    .block-container{
        max-width:1180px;
        padding:4.8rem 1.2rem 4rem !important
    }
    ...
    .route-arrow{color:#667085;padding:0 .15rem}
    .meta-row{
        display:flex;
        gap:.35rem;
        flex-wrap:wrap;
    ...
    @media(max-width:900px){
        .station-card{
            grid-template-columns:minmax(140px,.75fr) minmax(240px,1.25fr)
        }
        .summary-area{
            grid-column:1/-1;
...
for band in bands:
    ...
    for _, row in band_df.iterrows():
        ...
        transfer_chip = (
            ...
            if row["乗換回数"] == 0
            else f'<span class="chip">{escape(row["乗換駅"])}で乗換</span>'
        )
            ...
            <div class="summary-area">
                ...
                <details>
                    ...
                    <div class="details-body">
                        ...
                        <div>
                            <span class="detail-label">松戸到着：</span>
                            {row["到着"]}
                        </div>
                        <div>
                            <span class="detail-label">乗換駅：</span>
                            {transfer_detail}
                        </div>
                        <div>
