...
def merge_data(parts):
    ...
    for part in parts:
        ...
        for station in part.get("stations", []):
            ...
            current["routes"] = sorted(set(current["routes"] + station.get("routes", [])))
            current["operators"] = sorted(set(current["operators"] + station.get("operators", [])))

            if station.get("location") not in ("", "所在地未登録"):
                current["location"] = station["location"]
                current["municipality_code"] = station.get("municipality_code", "")
            ...
            if station.get("lat") is not None and station.get("lon") is not None:
                ...
                current["lat"] = lat if count == 0 else (current["lat"] * count + lat) / (count + 1)
                current["lon"] = lon if count == 0 else (current["lon"] * count + lon) / (count + 1)
                current["_coord_count"] = count + 1

        for day_type in timetables:
            timetables[day_type].extend(part.get("timetables", {}).get(day_type, []))
...
LINE_STYLES = {
    ...
    "つくばエクスプレス": ("#003894", "#eef4ff"), "多摩モノレール": ("#ff8a00", "#fff7ec"),
    "山手線": ("#9acd32", "#f5faec"), "京浜東北": ("#00b2e5", "#eefaff"),
    "根岸線": ("#00b2e5", "#eefaff"), "中央線快速": ("#f15a22", "#fff5ef"),
    "中央・総武": ("#ffd400", "#fffceb"), "総武線快速": ("#0067a5", "#eff7fc"),
    "横須賀線": ("#0067a5", "#eff7fc"), "埼京線": ("#00a65a", "#effaf4"),
    "川越線": ("#00a65a", "#effaf4"), "常磐線": ("#00a85a", "#effaf4"),
    "京葉線": ("#c9242f", "#fff1f2"), "武蔵野線": ("#f15a22", "#fff5ef"),
    "東海道線": ("#f68b1f", "#fff6ed"), "湘南新宿": ("#e87511", "#fff6ed"),
    "上野東京": ("#7a5c3e", "#f8f4ef"),
}
...
def nearby_stations(destination, radius):
    ...
    for name, info in station_info.items():
        ...
        if distance <= radius:
            result.append({"name": name, "distance": distance, "walk": estimated_walk_minutes(distance)})
...
def line_style(route):
    return next((style for name, style in LINE_STYLES.items() if name in str(route)), DEFAULT_STYLE)
        ...
        for destination_index, destination_stop in enumerate(stops):
            ...
            for origin in stops[:destination_index]:
                ...
                candidate = {
                    ...
                    "家賃": info.get("rent"), "出発": clock(departure), "到着": clock(arrival),
                    "到着駅": destination, "徒歩時間": walk, "目的地到着": clock(arrival + walk),
                    "乗車時間": arrival - departure, "所要時間": arrival + walk - departure,
                    "経路": trip.get("route", "路線情報なし"), "事業者": trip.get("operator", ""),
                    "行先": trip.get("destination", "行先情報なし"), "路線一覧": info.get("routes", []),
                    "出発分": departure,
                }
                current = latest.get(station)
                if current is None or departure > current["出発分"] or (
                    departure == current["出発分"] and candidate["所要時間"] < current["所要時間"]
                ):
...
# ============================================================
# 4. 「住む」画面：直通の実乗車時間だけを使う
# ============================================================
@st.cache_resource
def build_station_trip_index():
    # 平日時刻表を駅別に索引化し、中心駅に停車する列車だけを調べる
    index = {}
    for trip in timetables.get("weekday", []):
        stops = trip.get("stops", [])
        for i, stop in enumerate(stops[:-1]):
            if len(stop) >= 3 and stop[1] and stop[2]:
                index.setdefault(stop[0], []).append((trip, i))
    return index


def direct_life_access(origins, major_count):
    # 待ち時間・発車時刻・乗換を除外し、直通列車の実乗車時間を重複排除
    major = {name for name, info in station_info.items() if len(info["routes"]) >= major_count}
    index, rows = build_station_trip_index(), {}

    for origin, walk in origins.items():
        for trip, i in index.get(origin, []):
            stops, departure = trip.get("stops", []), minute(trip["stops"][i][2])
            route = trip.get("route", "路線情報なし")
            for later in stops[i + 1:]:
                if len(later) < 2 or not later[1]:
                    continue
                target, ride = later[0], minute(later[1]) - departure
                if target not in major or target in origins or not 0 < ride <= 60:
                    continue
                total = ride + walk
                if total > 60:
                    continue
                key = (target, origin, route, ride, walk)
                rows[key] = {
                    "name": target, "from": origin, "route": route,
                    "ride": ride, "walk": walk, "minutes": total,
                }

    # 同じ到着駅・出発駅・路線で複数の実乗車時間がある場合、最短だけ（速）表示
    groups = {}
    for row in rows.values():
        groups.setdefault((row["name"], row["from"], row["route"]), set()).add(row["ride"])
    for row in rows.values():
        times = groups[(row["name"], row["from"], row["route"])]
        row["fast"] = len(times) > 1 and row["ride"] == min(times)

    return sorted(rows.values(), key=lambda x: (x["minutes"], x["name"], x["route"], x["ride"]))


def map_positions(home, access):
    # 半径は時間に固定し、重なり回避では角度だけを動かす
    ...
    for row in access:
        ...
        radius = row["minutes"] / 60 * 50  # 60分円の半径を50%とする

        # 半径は変えず、左右へ少しずつ角度をずらして衝突を避ける
        candidates = [0]
        for step in range(1, 13):
            candidates += [step * 4, -step * 4]

        chosen = None
        for shift in candidates:
            angle = base_angle + math.radians(shift)
            x, y = 50 + math.sin(angle) * radius, 50 - math.cos(angle) * radius
            if all(abs(x - old["x"]) >= 8 or abs(y - old["y"]) >= 5 for old in placed):
                chosen = (x, y)
                break

        x, y = chosen or (
            50 + math.sin(base_angle) * radius,
            50 - math.cos(base_angle) * radius,
        )
        placed.append({**row, "x": x, "y": y})
    return placed


def life_path(home):
    raw = st.query_params.get("life_path", "")
    path = [name for name in raw.split("|") if name in station_info]
    return path if path and path[-1] == home else [home]


def life_url(path):
    return f"?live_station={quote(path[-1])}&life_path={quote('|'.join(path))}"
...
def render_life_screen(home):
    ...
    with top1:
        if len(path) > 1:
            st.markdown(f'<a class="back-link" href="{life_url(path[:-1])}">← {escape(path[-2])}へ戻る</a>', unsafe_allow_html=True)
        else:
            st.markdown('<a class="back-link" href="?">← 検索結果へ戻る</a>', unsafe_allow_html=True)
    with top2:
        crumbs = " ＞ ".join(
            f'<a href="{life_url(path[:i + 1])}">{escape(name)}</a>'
            for i, name in enumerate(path)
        )
        st.markdown(f'<div class="life-crumbs">{crumbs}</div>', unsafe_allow_html=True)

    st.markdown(f'<div class="page-title">{escape(home)}から直通で何分？</div>', unsafe_allow_html=True)
    st.caption("電車に乗った瞬間からの実乗車時間です。待ち時間・発車時刻・運行間隔・乗換は含みません。")

    settings, note = st.columns([1, 2.2])
    with settings:
        with st.popover("⚙ 詳細設定", width="stretch"):
            ranges = {"徒歩2分": 100, "徒歩5分": 300, "徒歩8分": 500, "徒歩12分": 700, "徒歩17分": 1000}
            radius_label = st.selectbox(
                "利用可能な近隣駅を探す範囲", list(ranges), index=2, key=f"life_radius_{home}"
            )
            candidates = nearby_stations(home, ranges[radius_label])
            st.caption("近隣駅を使う場合は、徒歩時間を乗車時間に加えて円へ配置します。")
            for candidate in candidates:
                name = candidate["name"]
                c1, c2 = st.columns([3, 1])
                c1.checkbox(name, value=candidate["distance"] <= 500, key=f"life_use_{home}_{name}")
                c2.number_input(
                    "分", 1, 30, candidate["walk"], key=f"life_walk_{home}_{name}",
                    label_visibility="collapsed"
                )
            st.radio(
                "主要駅の基準", [3, 2], format_func=lambda x: f"{x}路線以上",
                horizontal=True, key=f"life_major_{home}"
            )
    ...
    with note:
        st.caption("利用駅：" + "、".join(
            f"{name}{f'（徒歩{walk}分）' if walk else ''}" for name, walk in origins.items()
        ))

    major_count = st.session_state.get(f"life_major_{home}", 3)
    access = direct_life_access(origins, major_count)
    points = map_positions(home, access)
    rings = "".join(f'<div class="life-ring r{n}"><span>{n}分</span></div>' for n in (15, 30, 45, 60))
    labels = ""

    for point in points:
        name, route = escape(point["name"]), escape(point["route"])
        accent, background = line_style(point["route"])
        fast = "（速）" if point["fast"] else ""
        detail = (
            f'{escape(point["from"])}から{route}<br>'
            f'乗車{point["ride"]}分'
            + (f'＋徒歩{point["walk"]}分' if point["walk"] else "")
        )
        next_path = path + [point["name"]]
        labels += f"""<details class="map-point" style="left:{point['x']:.1f}%;top:{point['y']:.1f}%;
        --line-color:{accent};--card-bg:{background}">
        <summary><span class="map-name">{name}{fast}</span><span class="map-minutes">{point['minutes']}分</span></summary>
        <div class="map-detail">{detail}<br>
        <a class="recenter-link" href="{life_url(next_path)}">{name}を中心に見る</a></div></details>"""

    st.markdown(
        compact_html(f'<div class="life-map">{rings}<div class="life-home">{escape(home)}</div>{labels}</div>'),
        unsafe_allow_html=True,
    )
    if not points:
        st.info("条件に合う主要駅がありません。近隣駅または主要駅の基準を調整してください。")
    st.caption("カードの中心位置が所要時間を示します。同じ駅でも実乗車時間が異なる場合は別カードで表示し、最短側に（速）を付けます。")
...
.page-title{font-size:clamp(1.25rem,2.3vw,1.85rem);font-weight:900;line-height:1.2;letter-spacing:-.04em}
.page-note{opacity:.58;font-size:.68rem;text-align:right;white-space:nowrap}
.challenge-notice{padding:.42rem .65rem;margin:.2rem 0 .55rem;border:1px solid #e5a00080;border-radius:9px;background:#fff8df;color:#694c00;font-size:.68rem}
.near-destination{padding:.6rem .72rem;margin:.35rem 0 .6rem;border:1px solid #98a2b360;border-radius:10px;background:rgba(128,128,128,.05)}
...
.st-key-nearby_toggle [data-testid="stMarkdownContainer"],.st-key-nearby_toggle [data-testid="stMarkdownContainer"] p{margin:0!important}
.nearby-row{display:flex;align-items:center;justify-content:space-between;height:34px;padding:0 .65rem;background:rgba(128,128,128,.08);border:1px solid rgba(128,128,128,.35);border-radius:9px;font-size:.7rem}
.st-key-nearby_toggle div[data-testid="stButton"] button{height:34px!important;min-height:34px!important;padding:0 .6rem!important;font-size:.68rem!important;white-space:nowrap;border-radius:9px!important}

.station-card{position:relative;display:grid;grid-template-columns:minmax(145px,1fr) minmax(190px,1.1fr) minmax(180px,1fr) 78px;grid-template-areas:"station departure route actions" "location arrival rent actions";align-items:center;gap:.08rem clamp(.7rem,1.8vw,1.4rem);color:#283141!important;background:var(--card-bg)!important;border:1px solid #98a2b380;border-left:6px solid var(--line-color)!important;border-radius:12px;padding:.62rem .85rem;margin-bottom:.42rem;box-shadow:0 1px 3px #0000000b}
...
.live-link,.back-link,.recenter-link{display:inline-flex;align-items:center;justify-content:center;border:1px solid #98a2b3;border-radius:8px;background:#fff;color:#344054!important;text-decoration:none;font-weight:850;white-space:nowrap}
.live-link{height:28px;padding:0 .45rem;font-size:.66rem}.back-link{padding:.34rem .5rem;font-size:.68rem}.recenter-link{margin-top:.35rem;padding:.28rem .42rem;font-size:.6rem}
...
.details-body{position:absolute;z-index:20;right:.8rem;top:3.2rem;width:min(390px,calc(100vw - 3rem));padding:.65rem .75rem;border:1px solid #d0d5dd;border-radius:9px;background:#fff;text-align:left;line-height:1.65;box-shadow:0 7px 20px #0002}
.detail-label{font-weight:850}.detail-divider{height:1px;background:#e4e7ec;margin:.35rem 0}
.empty{padding:2rem;text-align:center;border:1px dashed rgba(128,128,128,.6);border-radius:12px}

.life-crumbs{font-size:.68rem;opacity:.75;text-align:right}.life-crumbs a{color:inherit;text-decoration:none}
.life-map{position:relative;width:min(760px,94vw);aspect-ratio:1;margin:1rem auto 1.3rem;border-radius:50%;background:radial-gradient(circle,#fff 0,#f8fafc 100%);overflow:visible}
...
.map-point summary{display:flex;flex-direction:column;width:auto;height:auto;min-width:58px;padding:.26rem .42rem;border:1px solid #98a2b380;border-left:5px solid var(--line-color);border-radius:8px;background:var(--card-bg);box-shadow:0 1px 4px #0002;font-family:inherit;white-space:nowrap;line-height:1.08}
.map-name{font-size:.62rem;font-weight:900;color:#283141}.map-minutes{margin-top:.12rem;font-size:.68rem;font-weight:950;color:#283141}
.map-point .map-detail{position:absolute;z-index:30;left:50%;top:2.7rem;transform:translateX(-50%);width:max-content;max-width:240px;padding:.45rem .55rem;border:1px solid #d0d5dd;border-radius:8px;background:#fff;font-size:.6rem;line-height:1.45;box-shadow:0 5px 15px #0002;color:#344054}

@media(max-width:620px){
.block-container{padding:2.9rem .55rem 2.5rem!important}
div[data-testid="stHorizontalBlock"]{display:flex!important;flex-direction:row!important;flex-wrap:nowrap!important;gap:.3rem!important}
div[data-testid="stHorizontalBlock"]>div{min-width:0!important}
.selector-label{font-size:.59rem;margin-bottom:.08rem}.heading-row{display:block;margin:.4rem 0 .42rem}
.page-title{font-size:1rem}.page-note{margin-top:.12rem;font-size:.56rem;text-align:left;white-space:normal}
.challenge-notice{font-size:.57rem;padding:.35rem .45rem}.near-destination{padding:.45rem .5rem}
.nearby-row{height:31px;padding:0 .42rem;font-size:.58rem}
.st-key-nearby_toggle div[data-testid="stButton"] button{height:31px!important;min-height:31px!important;padding:0 .35rem!important;font-size:.58rem!important}
...
.station-name{font-size:1.2rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.location{font-size:.57rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.departure-wrap{display:block}.clock{display:none}.departure{font-size:1.25rem;line-height:1}.arrival{padding:0;font-size:.54rem}
.rent{position:relative;padding-right:22px;font-size:.58rem}.route{font-size:.57rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.actions-area{position:absolute;top:.31rem;right:.3rem}.live-link{height:20px;padding:0 .32rem;font-size:.55rem}
summary{width:20px;height:20px;font-size:.6rem}.details-body{position:relative;right:auto;top:auto;width:auto;margin-top:.35rem;box-shadow:none}
.life-map{width:92vw}.map-point summary{min-width:50px;padding:.2rem .3rem}.map-name{font-size:.5rem}.map-minutes{font-size:.57rem}.life-home{font-size:.64rem}
.life-crumbs{font-size:.56rem}.back-link{font-size:.56rem;padding:.28rem .36rem}
}
</style>
""",
    unsafe_allow_html=True,
...
destination_col, time_col, filter_col = st.columns([2.5, 1.05, 0.38])

with destination_col:
    st.markdown('<div class="selector-label">勤務先・目的駅</div>', unsafe_allow_html=True)
    selected_destination = st.selectbox(
        "勤務先・目的駅", stations, index=None, placeholder="神保町（駅名・路線で検索）",
...
with time_col:
    ...
    arrival_time = st.time_input("到着時刻", value=time(8), step=60, label_visibility="collapsed")
...
if nearby_mode:
    ...
    radius = st.select_slider(
        "近隣駅として探す範囲", options=[300, 400, 500, 600, 800, 1000],
        value=500, format_func=lambda value: f"{value}m",
    )
    ...
    if not candidates:
        ...
        st.markdown(
            f'<div class="near-destination-title">近くの駅を{len(candidates)}駅見つけました</div>'
        ...
        for candidate in candidates:
            ...
            with use_col:
                use_station = st.checkbox(f"{name}（直線約{distance}m）", value=True, key=f"use_{key_base}")
            with walk_col:
                walk = st.number_input(
                    f"{name}から目的地まで", min_value=1, max_value=30, value=estimated,
                    step=1, key=f"walk_{key_base}", label_visibility="collapsed",
                )
            if use_station:
                destination_options[name] = int(walk)
    st.markdown("</div>", unsafe_allow_html=True)

weekday_df = pd.DataFrame(search_routes(timetables.get("weekday", []), destination_options, target_minute))
saturday = {row["駅名"]: row for row in search_routes(timetables.get("saturday", []), destination_options, target_minute)}
sunday = {row["駅名"]: row for row in search_routes(timetables.get("sunday", []), destination_options, target_minute)}
...
# ============================================================
st.markdown(
    compact_html(f"""<div class="heading-row"><div class="page-title">{escape(destination)}に{target}までに着くには？</div>
    <div class="page-note">平日・直通のみ｜対象 {escape(service_dates.get("weekday", ""))}</div></div>"""),
    unsafe_allow_html=True,
)

if len(destination_options) > 1:
    st.caption("到着候補：" + "、".join(destination_options) + "。近隣駅は設定した徒歩時間を差し引いて検索しています。")

if config.get("jr_enabled"):
    st.markdown(
        '<div class="challenge-notice">JR東日本の公共交通オープンデータチャレンジ2026限定データを利用しています。</div>',
        unsafe_allow_html=True,
    )
...
if nearby_count or st.session_state.show_nearby:
    with st.container(key="nearby_toggle"):
        ...
        with notice_col:
            message = (
                f"<strong>近距離の出発駅も表示中</strong><span>10分未満の{nearby_count}駅を含む</span>"
                if st.session_state.show_nearby
                else f"<strong>少し離れた候補を優先して表示中</strong><span>10分未満の{nearby_count}駅を省略</span>"
            )
            st.markdown(compact_html(f'<div class="nearby-row">{message}</div>'), unsafe_allow_html=True)
        with button_col:
            st.button(
...
if df.empty:
    ...
    for _, row in df.iterrows():
        ...
        station, location, route = escape(raw_station), escape(str(row["所在地"])), escape(str(row["経路"]))
        operator, train_destination = escape(str(row["事業者"])), escape(str(row["行先"]))
        routes, arrival_station = escape(" ／ ".join(row["路線一覧"])), escape(str(row["到着駅"]))
        walk, accent, background = int(row["徒歩時間"]), *line_style(row["経路"])
        ...
        arrival_text = (
            f'{arrival_station} {row["到着"]}着 → 徒歩{walk}分 → {escape(destination)} {row["目的地到着"]}'
            if walk else f'{escape(destination)} {row["到着"]}着'
        )

        card = f"""<div class="station-card" style="--line-color:{accent};--card-bg:{background}">
        <div class="station-name">{station}</div><div class="location">{location}</div>
        <div class="departure-wrap">{clock_html(str(row["出発"]))}<div class="departure">{row["出発"]}発</div></div>
        <div class="arrival">{arrival_text}</div><div class="rent">家賃：{row["家賃区分"]}</div>
        <div class="route">{route}・直通</div><div class="actions-area">
        <div class="details-area"><details><summary>i</summary><div class="details-body">
        <div><span class="detail-label">所在地：</span>{location}</div>
        <div><span class="detail-label">乗り入れ路線：</span>{routes}</div>
        <div><span class="detail-label">運行事業者：</span>{operator or "情報なし"}</div>
        ...
        <div class="detail-divider"></div>
        <div><span class="detail-label">平日：</span>{row["出発"]}発（{arrival_text}・{route}）</div>
        <div><span class="detail-label">土曜：</span>{escape(day_text(sat))}</div>
        ...
        <a class="live-link" href="?live_station={quote(raw_station)}&life_path={quote(raw_station)}">住む</a>
        </div></div>"""
...
st.caption("所在地と近隣駅の距離はGTFS座標と国土地理院の情報を基に算出しています。徒歩時間は直線距離からの概算で、実際の経路や駅構内移動は含みません。")
